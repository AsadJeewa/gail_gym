...
class PPOTrain:
    def __init__(self, Policy, Old_Policy, gamma=0.95, clip_value=0.2, c_1=1, c_2=0.01):
        ...
        :param c_2: parameter for self.entropy bonus
        """
        ...
        with tf.variable_scope('loss'):
            ...
            loss_vf = tf.squared_difference(self.rewards + self.gamma * self.v_preds_next, v_preds)#Returns (x - y)(x - y) element-wise.
    ...
    def train(self, obs, actions, gaes, rewards, v_preds_next):
        #return tf.get_default_session().run([self.train_op,self.loss_out,self.loss_clip,self.loss_vf,self.entropy], feed_dict={self.Policy.obs: obs,
        return tf.get_default_session().run([self.train_op,self.loss_out], feed_dict={self.Policy.obs: obs,
